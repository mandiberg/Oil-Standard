...
//
// UPDATE ON 06/28/18:
// Now the script runs on both Greasemonkey+Firefox setup and Tampermonkey+Chrome setup.
// To install on Chrome you will need Tampermonkey: http://tampermonkey.net/
// In tampermonkey add-on you will need to create a new script and copy paste this code into the editor.
// After saving the script you will need to make sure that the script is enabled in Tampermonkey dashboard.
// We updated the data feed to http://www.cmegroup.com/CmeWS/mvc/Quotes/Future/424/G? which returns a json object
// --------------------------------------------------------------------
...
function getOilPrices() {
  ...
  GM_xmlhttpRequest({
    method: 'GET',
    url: 'http://www.cmegroup.com/CmeWS/mvc/Quotes/Future/424/G?',

    onload: function(responseDetails) {
      var obj = JSON.parse(responseDetails.responseText);
      // alert(obj.quotes[0].last);
      GM_setValue("barrelprice", parseFloat(obj.quotes[0].last));
